...
class extension_test extends \advanced_testcase {
     ...
     * @covers       \mod_bigbluebuttonbn\extension::get_class_implementing
     */
    public function test_get_class_implementing(bool $bbbenabled, string $apiclass, array $extensionclasses): void {
        ...
        $getclassimplementing = $reflectionextension->getMethod('get_class_implementing');
        $getclassimplementing->setAccessible(true);
        $allfoundclasses = $getclassimplementing->invoke(null, $apiclass);
        $this->assertEquals($extensionclasses, $allfoundclasses);
    }
    ...
    public function classes_implementing_class(): array {
        return [
            'mod_form_extension with plugin disabled' => [
                'bbbenabled' => false,
                'apiclass' => mod_instance_helper::class,
                'result' => []
            ],
            'mod_form_extension with plugin enabled' => [
                'bbbenabled' => true,
                'apiclass' => mod_instance_helper::class,
                'result' => [
