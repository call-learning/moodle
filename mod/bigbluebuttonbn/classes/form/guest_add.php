...
class guest_add extends dynamic_form {
    ...
    public function validation($formdata, $files): array {
        ...
        if (!empty($formdata['emails'])) {
            ...
            foreach ($emails as $email) {
                ...
                if (!empty(\core_user::get_user_by_email($email))) {
    ...
    protected function get_instance_from_params(): instance {
        ...
        if (empty($bbid)) {
            throw new \restricted_context_exception("No BigblueButton instance ID provided");
        }
    ...
    protected function get_page_url_for_dynamic_submission(): moodle_url {
        $context = $this->get_context_for_dynamic_submission();
        return new moodle_url('/mod/bigbluebuttonbn/view.php', ['id' => $context->instanceid, 'sesskey' => sesskey()]);
    }
}
