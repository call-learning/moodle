...
class admin_plugin_manager {
    ...
    public function execute(?string $action = null, ?string $plugin = null): void {
        ...
        // Process.
        $actionname = $action . '_plugin';
        if (method_exists($this, $actionname) && !empty($plugin)) {
            $action = $this->$actionname($plugin);
        }
        // View.
        if ($action == 'view') {
    ...
    private function view_plugins_table() {
        ...
        $table->define_columns(array('pluginname', 'version', 'hideshow', 'order',
            'settings', 'uninstall'));
        $table->define_headers(array(get_string('subplugintype_bbbext', 'mod_bigbluebuttonbn'),
        ...
        $shortsubtype = substr(extension::BBB_EXTENSION_PLUGIN_NAME, strlen('mod_bigbluebuttonbn'));
        ...
        foreach ($plugins as $idx => $plugin) {
            ...
            $row[] = get_string('pluginname', extension::BBB_EXTENSION_PLUGIN_NAME . '_' . $plugin);
            $row[] = get_config(extension::BBB_EXTENSION_PLUGIN_NAME . '_' . $plugin, 'version');

            $visible = !get_config(extension::BBB_EXTENSION_PLUGIN_NAME . '_' . $plugin, 'disabled');
            ...
            $exists = file_exists($typebasedir . $shortsubtype . '/' . $plugin . '/settings.php');
            if ($row[1] != '' && $exists) {
                $row[] = html_writer::link(new moodle_url('/admin/settings.php',
                    array('section' => extension::BBB_EXTENSION_PLUGIN_NAME . '_' . $plugin)), get_string('settings'));
            } else {
                $row[] = '&nbsp;';
            }
    ...
    private function format_icon_link(string $action, string $plugin, string $icon, string $alt): string {
        ...
        if ($action === 'delete') {
            $url = core_plugin_manager::instance()->get_uninstall_url(
                extension::BBB_EXTENSION_PLUGIN_NAME . '_' . $plugin,
                'manage');
            if (!$url) {
                return '&nbsp;';
            }
     ...
     */
    private function hide_plugin(string $plugin) {
     ...
     * @return string The next page to display
     */
    private function show_plugin(string $plugin) {
        $class = \core_plugin_manager::resolve_plugininfo_class(extension::BBB_EXTENSION_PLUGIN_NAME);
     ...
     * @return string The next page to display
     */
    private function moveup_plugin(string $plugintomove) {
     ...
     * @return string The next page to display
     */
    private function movedown_plugin(string $plugintomove): string {
